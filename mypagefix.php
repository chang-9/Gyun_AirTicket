...
<p> 닉네임 </p>
<input type="text" class="form-control" id="id" name="id" value="<? echo $_SESSION['nick']?>">
<p>현재 닉네임을 확인하시고 변경하실 닉네임을 입력해주세요.</p>


<!--이메일변경 -->
<span style="color:white">이메일</span>
<input type="text" class="form-control" id="id" name="id" value="<? echo $_SESSION['email']?>">
<p>현재 이메일을 확인하시고 변경하실 이메일을 입력해주세요.</p>
<button type="submit" class="btn btn-primary submit mb-1" onclick="possiblenick()">변경</button>
...
function possiblenick(){ 
    ...
    window.open(url,'','width=450,height=150,left=500');     
}
</script>
...
function possibleemail(){ 
    ...
    var url = "./API/email_send.php?email="+email;     
    window.open(url,'','width=450,height=150,left=500');
</script>
